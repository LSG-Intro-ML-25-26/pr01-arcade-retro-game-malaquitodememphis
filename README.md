# 🌿⚡ CYBER-DRUID: El reinici
### *Shooter d'aventura en Pixel Art Retro-Cyberpunk*

Benvingut a **CYBER-DRUID**, una experiència d’acció i exploració en vista cenital on la natura ancestral i la tecnologia corrupta xoquen dins d’un servidor viu.

---

## 🎮 Descripció del Projecte

**CYBER-DRUID** és un videojoc d’aventura amb mecàniques de shooter, ambientat dins de **GAIA-PRIME**, un sistema digital creat per simular una biosfera perfecta… fins que una plaga de malware el va infectar completament.

El jugador encarna el **Cyber Druida**, l’última defensa del sistema.

---

## 👥 Integrants del Projecte

Aquest projecte ha estat desenvolupat per:

- **Francis Moreta** *(Maps designer and UI)*  
- **Eric Lorenzo** *(Logic developer)*

---

## 📖 Lore del Joc

### 🌍 GAIA-PRIME: El món prefecte digital

GAIA-PRIME és un servidor massiu dissenyat per mantenir un equilibri perfecte entre vida artificial i natura simulada.  
Però una infecció propagada pel **Kernel Corrupte** ha corromput el seu codi intern.

Ara, tot el sistema està a punt de col·lapsar.

---

## 🧙 El Protagonista: El Cyber-Druida

El jugador controla el **Cyber Druida**, un antivirus ancestral oblidat que desperta com a última mesura de seguretat davant del Kernel Corrupte.

- Te una túnica antiga amb circuits brillants
- L'arma es la **Cyber Gun**

Asset Cyber Druida: 

![Cyber Druida](Cyber-Druida_Assets/cyberdruid.png)

![Cyber Druida amb arma](Cyber-Druida_Assets/cyberdruid_with_gun.png)

![Cyber Druida_esquena](Cyber-Druida_Assets/cyberdruid_back_side.png)

![Cyber Druida dreta](Cyber-Druida_Assets/cyberdruid_right_side.png)

![Cyber Druida esquerra](Cyber-Druida_Assets/cyberdruid_left_side.png)

![Cyber Druida projectil](Cyber-Druida_Assets/cyberdruid_shoot.png)

## 👾 Enemics: La Plaga Carmesí

### Inse-Glitch

Aquest enemics son petites entitats erràtiques semblants a insectes digitals.

- Molt ràpids
- Fràgilsç

Asset Inse-Glitch:

![Inse-Glitch](Cyber-Druida_Assets/inseglitch.png)

### El Troyà

Es una massa corrupta i resistent, creada per bloquejar l’avanç del jugador.

- Lent
- Més vida

Asset El Troyà:

![El Troyà](Cyber-Druida_Assets/troyanvirus.png)

### Boss Final: El Kernel Corrupte

El Kernel Corrupte és una entitat gegantina:

- Representa el cor del sistema i la última defensa de la plaga carmesí.
- Te forma de una caixa fort amb una aparença perillosa.
- El seu atac és una bola de foc que et persegueix fins que xocar amb alguna cosa.

Asset Kernel Corrupte:

![Kernel Corrupte](Cyber-Druida_Assets/kernelcorrupte.png)

![Kernel Corrupte projectil](Cyber-Druida_Assets/boss_shoot.png)

## Nivells

### Nivell 1: Entrenament

Tutorial de iniciació: moviment, dispar i obtenció del bastó.

Tilemap:

![Nivell 1](Cyber-Druida_Assets/level1_map.png)

### Nivell 2: Zona Corrupte

Zona de combat dividida en dues parts:

- Arena amb emboscada d’Inse-Glitch.
- Laberint amb la clau y porta, patrullat per Troyans

Tilemap:

![UNivell 2](Cyber-Druida_Assets/level2_map.png)

### Nivell 3: Arena del Kernel Corrupte

Combats contra el Kernel Corrupte per salvar a tota la GAIA-PRIME.

Tilemap:

![Nivell 3](Cyber-Druida_Assets/level3_map.png)

Tiles dels nivells:

### Terra de sales

![Terra de sales](Cyber-Druida_Assets/base_floor.png)

### Terra de camins

![Terra de camins](Cyber-Druida_Assets/way_floor.png)

### Mur

![Mur](Cyber-Druida_Assets/walls.png)

### Mur cantonada

![Muros esquineros](Cyber-Druida_Assets/corner_walls.png)

### Cofre

![Cofre](Cyber-Druida_Assets/chest.png)

### Tarjeta d'accés

![Tarjeta de acceso](Cyber-Druida_Assets/acces_card.png)

### Portes d'accés

![Puertas de acceso](Cyber-Druida_Assets/acces_doors.png)

### Mur bloquejat

![Muro bloqueado](Cyber-Druida_Assets/block_wall.png)

## Demo Speed-Run del Joc

A continuació es pot veure un vídeo demostratiu del gameplay en format speed-run:

Video demo:

https://drive.google.com/file/d/1jwPNfTUsxoF_6A2rEdAcZ30HEMAXbPfM/view?usp=sharing
